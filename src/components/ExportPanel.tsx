...
export default function ExportPanel({ screenshots, selectedPresets }: ExportPanelProps) {
  ...
  const handleExport = async () => {
    if (!screenshots.length || !presets.length) {
      toast({ title: "Nothing to export", description: "Add screenshots and select dimensions first.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Export complete!", description: `${screenshots.length * presets.length} images exported.` });
    } catch (err) {
      console.error(err);
    ...
    <div className="flex items-center gap-4 p-4 rounded-xl border border-border surface-elevated">
      <div className="flex-1">
        ...
        <p className="text-xs text-muted-foreground">
          {screenshots.length} screenshot{screenshots.length !== 1 ? "s" : ""} × {presets.length} dimension{presets.length !== 1 ? "s" : ""} ={" "}
          <span className="text-primary font-medium">{screenshots.length * presets.length} images</span>
        </p>
      </div>
      <Button
